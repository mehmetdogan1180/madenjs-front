    ...
    <el-table
      ...
      <el-table-column
        ...
        <template slot-scope="{row}">
          {{$moment(row.date).format("DD.MM.YYYY dddd")}}
        </template>
      </el-table-column>
      <el-table-column
        prop="startHour"
...
<script>
const { scoring } = require('@/api/staff');
  ...
  methods: {
    async getScoring() {
      try {
        ...
        this.table.data = Object.keys(response.scorings).map((date) => {
          const r = {};
          const i = response.scorings[date];
          r.date = date;
